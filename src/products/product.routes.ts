...
productRouter.put("/product/:id", async (req: Request, res: Response) => {
  try {
    ...
    const existingProduct = await database.findOne(id);

    if (!existingProduct) {
      return res.status(StatusCodes.NOT_FOUND).json({ error: "Product does not exist." });
    }

    const newProduct = await database.update(id, updatedProduct);

    return res.status(StatusCodes.OK).json(newProduct);
  } catch (error) {
    console.error("Error updating product:", error);
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: "An error occurred while updating the product." });
  }
});
...
productRouter.delete("/product/:id", async (req: Request, res: Response) => {
  try {
    ...
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: "An error occurred while deleting the product." });
  }
});
